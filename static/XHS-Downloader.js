...
3. 提取账号发布、收藏、点赞、专辑post_link时，脚本可以自动滚动页面直至加载全部作品，默认滚动检测间隔：2.5 秒
4. 提取发现post_link、搜索作品、用户链接时，脚本可以自动滚动页面加载更多内容，默认滚动页面次数：10 次
...
`);
    ...
    const buttons = [createButton("Download", "下载无水印作品文件", extractDownloadLinks), createButton("Post", "提取发布post_link", extractAllLinksEvent, 0), createButton("Collection", "提取收藏post_link", extractAllLinksEvent, 1), createButton("Favorite", "提取点赞post_link", extractAllLinksEvent, 2), createButton("Feed", "提取发现post_link", extractAllLinksEvent, -1), createButton("Search", "提取搜索post_link", extractAllLinksEvent, 3), createButton("User", "提取搜索用户链接", extractAllLinksEvent, 4), createButton("Board", "提取专辑post_link", extractAllLinksEvent, 5), createButton("Disclaimer", "脚本说明及免责声明", readme,), createButton("About", "关于 XHS-Downloader", about,),];
